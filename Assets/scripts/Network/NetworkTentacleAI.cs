...
public class NetworkTentacleAI : NetworkBehaviour
{
    ...
    public IEnumerator VacuumMicrobe(Collider2D other)
    {
        ...
        while ((other != null) && (Vector2.Distance(other.transform.position, characterCenter.position) > 0.1f))
        {
            other.transform.position = Vector2.MoveTowards(
                other.transform.position,
                characterCenter.position,
                1f * stretchSpeed * Time.deltaTime
            );
            yield return null;
        }
        if (other == null)
        {
            yield break; // Exit if the other object is destroyed
        }
        ...
        if (other.CompareTag("Ecoli"))
        {
            ...
            // Call the server RPC to kill the Ecoli
            KillEcoliServerRpc(other.GetComponent<NetworkObject>());
        }
    ...
    void KillEcoliServerRpc(NetworkObjectReference ecoliRef)
    {
        if (ecoliRef.TryGet(out NetworkObject ecoliNetObj))
        {
            if (ecoliNetObj.TryGetComponent<NetworkEcoliAI>(out NetworkEcoliAI ecoli))
            {
                ecoli.Die();
            }
        }
